<script lang=ts>
  ...
  const navItems = $derived.by(() => {
    ...
    if (perms.includes('fulfillment') || perms.includes('admin')) {
    ...
    <div class="sidebar-footer">
      ...
      <div class="user-info">
        <p class="username">{displayName || 'Guest'}</p>
          <p class="permission">{perms.join(' · ').replaceAll('-', ' ')}</p>
...
<style>
  ...
  .sidebar {
    ...
    top: 0;
    width: 250px;
    background: linear-gradient(135deg, var(--background-700) 0%, var(--background-800) 50%, var(--background-700) 100%);
